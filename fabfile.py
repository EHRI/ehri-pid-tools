"""
Fabric deployment script for EHRI Pid Tools webapp.
"""
...
deploys_dir = "/opt/ept/deploys"
target_link = "/opt/ept/target"
...
def deploy(ctx, clean=False):
    ...
    local(build_cmd)
    file = local(f"ls -1t target/universal/ept-*.zip").stdout.strip()
...
@task
def restart(ctx):
    """Restart the ept process"""
    ctx.run("sudo service ept restart")
